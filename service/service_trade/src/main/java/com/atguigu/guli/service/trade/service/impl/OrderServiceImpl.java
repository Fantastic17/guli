...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public String saveOrder(String courseId, String memberId) {
        ...
        order.setTotalFee(courseDto.getPrice().multiply(new BigDecimal(100))); //单位：分 换算成元

        order.setMemberId(memberId);
        order.setNickname(memberDto.getNickname());
        order.setMobile(memberDto.getMobile());

        order.setStatus(Order.ORDER_NOT_PAID); // 未支付
    ...
    public void updateOrderStatus(Map<String, String> notifyMap) {
        ...
        order.setStatus(Order.ORDER_PAY); // 修改为已支付
        ...
        payLog.setTotalFee(Long.parseLong(notifyMap.get("total_fee")));// 单位：分
